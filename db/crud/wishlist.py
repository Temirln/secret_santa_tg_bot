...
async def delete_user_wish(session, wish_id):
    async with session() as session:
        async with session.begin():
            stmt = delete(WishList).where(WishList.id == wish_id)
...
async def get_user_wishes(session, user_tg_id):
    async with session() as session:
        async with session.begin():
            stmt = select(WishList).where(WishList.tg_user_id == user_tg_id)
...
async def update_user_wish(session, user_tg_id, wish_id):
    async with session() as session:
        async with session.begin():
            stmt = (
                update(WishList)
                .where(WishList.tg_user_id == user_tg_id, WishList.id == wish_id)
                .values(is_gift_received=True)
            )
